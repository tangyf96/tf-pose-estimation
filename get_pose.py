...
def feature_extraction(human_info,human_boxes,count):
    persons=[]
    count=0
    for i in  range(len(human_info)):
        some_human={"id":count,"body_points":human_info[i],"last_update":count,"feature":human_boxes[i]}
...
def draw_persons(npimg,count,persons,color):
    ...
    return npimg
def find_min(first_human,persons,count):
...
if __name__ == '__main__':
    ...
    for im_file in TEST_IMAGE_PATHS:
        ...
        (image,human_info,human_boxes) = TfPoseEstimator.draw_humans(im_file, image, humans, imgcopy=False)
        temp_info=feature_extraction(human_info,human_boxes,count)
        if count==0:
            ...
            if range(len(temp_info))<=range(len(persons)):
                for i in range(len(temp_info)):
                    persons[i]=find_min(temp_info[i],persons,count)
                image=draw_persons(image,count,persons,color)
                cv2.imshow("test", image)
                cv2.waitKey(2)
        count=count+1
